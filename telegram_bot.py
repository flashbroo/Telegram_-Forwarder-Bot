...
from force_subscribe import check_join_status, join_keyboard, get_force_message, missing_channels_text
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not config.is_admin(user.id) and config.get_force_sub_channels():
        join_status = await check_join_status(context.bot, user.id)
        if not join_status["ok"]:
            await update.message.reply_text(
                get_force_message() + missing_channels_text(join_status),
...
async def check_join_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not config.is_admin(user.id):
        join_status = await check_join_status(context.bot, user.id, use_cache=False)
    else:
        join_status = {"ok": True, "missing": []}

    if not join_status["ok"]:
        await q.answer("Join required. Check the missing channel list.", show_alert=True)
        try:
